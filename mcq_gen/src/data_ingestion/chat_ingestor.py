...
class ChatIngestor:
    def __init__( 
        ...
        try:
            ...
            raise ProjectException("Initialization error in ChatIngestor", e)
    ...
    def _splitter(self, docs: List[Document], chunk_size=1000, chunk_overlap=200) -> List[Document]:
    ...
    def build_retriever(
        ...
        *,
        ...
        try:
            paths = save_uploaded_files(uploaded_files, self.temp_dir) # load files from source and save in data dir with clean file names
            docs = load_documents(paths) # load those saved files
            if not docs:
                raise ValueError("No valid documents loaded")
            
            # chunking
            chunks = self._splitter(docs, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
            ...
            raise ProjectException("Failed to build retriever", e)
